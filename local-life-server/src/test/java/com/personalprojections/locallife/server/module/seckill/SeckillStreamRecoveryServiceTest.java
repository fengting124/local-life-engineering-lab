...
class SeckillStreamRecoveryServiceTest {
    ...
    void recoverOnePersistsReservationAndWritesOutbox() {
        ...
        verify(reservationMapper).insertIfAbsent(captor.capture());
        ...
        verify(outboxService).saveToOutboxIfAbsent(any(), eq("100_300_seckill"), any(), any());
    }

    @Test
    void existingReservationStillAttemptsOutboxRecovery() {
        SeckillStreamRecoveryService service = newService();
        when(reservationMapper.insertIfAbsent(any(SeckillReservation.class))).thenReturn(0);
