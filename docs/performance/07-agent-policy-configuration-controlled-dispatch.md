...
| 门禁 | 结果 |
| --- | --- |
| Agent 完整行为测试 | 852/852 passed |
| Agent 覆盖率 | 81.66%（门槛 45%） |
...
完整 Agent 测试在隔离的 Python 3.11 CI 镜像中运行。其中主测试集为 851 passed，
Embedding 镜像中的模型依赖测试为 1 passed；两者合计 852/852。宿主机旧虚拟环境仍是
